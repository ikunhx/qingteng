        ...
        <div v-if="currentPage === 'administrator'" class="administrator">
            <div class="table-container1">
                <el-table :data="userData" style="width: 100%" :row-class-name="tableRowClassName">
                    <el-table-column prop="ranking" label="排名" width="180" class="ranking"></el-table-column>
            ...
            <el-button class="superAdministrator-button" type="primary" @click="toSuperAdministrator">超级管理员</el-button>
        </div>
        <div v-else-if="currentPage === 'superAdministrator'" class="superAdministrator">
            <div class="table-container2">
                <el-table-column>
                    <el-input class="inputBox" v-model="email" placeholder="请输入邮箱" />
                    <el-button class="add-button" size="mini" type="success"
                ...
                <el-button class="administrator-button" type="primary" @click="toAdministrator">返回</el-button>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将永久删除该管理员, 是否继续?', '提示', {
                ...
                axios.delete(`https://qingteng-recruitment/root/delete_administrator/${row.id}`)
                    .then(response => {
                        this.refreshAdministratorData();
        ...
        fetchUsers() {
            axios.post('https://qingteng-recruitment/root/select_user')
                .then(response => {
