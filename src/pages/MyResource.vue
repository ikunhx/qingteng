<template>
  <div>
    ...
    <el-dialog title="编辑资源" :visible.sync="dialogVisible">
      <el-form :model="form">
        ...
        <el-form-item label="资源文件">
          <el-upload class="upload-demo" drag action="#" :on-change="handleFileChange" :file-list="fileList">
    ...
    <el-drawer :with-header="false" :visible.sync="answerTable" direction="ltr" size="80%">
      <!-- 加载动画 -->
      <div v-if="!pdfFiles.length && !videoFiles.length" class="loading-container">
        <div class="spinner"></div>
      </div>

      <video v-for="(videoFile, index) in videoFiles" :key="index" :src="videoFile.url" fit="fill" controls
        style="width: 100%"></video>
      <div v-for="(pdfFile, index) in pdfFiles" :key="index" class="pdf-container">
        <embed :src="pdfFile.url" type="application/pdf" width="100%" height="80vh" class="pdf" />
      </div>
    </el-drawer>
...
export default {
  ...
  methods: {
    ...
    async fetchResources() {
      try {
        const response = await axios.post('http://localhost:8080/qingteng-recruitment/root/display_resource', { code: 1 })
  ...
  async editResources(id) {
    ...
    // 发送POST请求到服务器更新资源信息
    try {
      const response = await axios.post(`http://localhost:8080/qingteng-recruitment/root/resource_edit?id=${id}`, {
        name: this.form.name,
  ...
  async updateResource() {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/qingteng-recruitment/root/resource_edit?id=${this.form.id}`, formData)
      if (response === 200) {
  ...
  async publishResource(id) {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/qingteng-recruitment/root/resource_put?id=${id}`, payload)
  ...
  async deleteResource(id) {
    try {
      const response = await axios.post(`http://localhost:8080/qingteng-recruitment/root/resource_delete?id=${id}`)
      if (response.data.code === 200) {
        console.log('资源删除成功')
        this.fetchResources() // 刷新资源列表 
      }
